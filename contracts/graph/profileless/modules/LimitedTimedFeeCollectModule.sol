...
contract LimitedTimedFeeCollectModule is CollectModuleBase, ICollectModule {
    ...
    function processCollect(uint256 pubId, address collector, bytes calldata data) external onlyHub {
        if (block.timestamp > _publicationDataById[pubId].endTimestamp) {
            revert CollectExpired();
        }
